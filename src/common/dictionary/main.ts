...
import { eventName, eventYear } from '../settings.js';
import { DictionaryLabel, Language, LanguageLabel } from './types.js';

const labels: DictionaryLabel = {
    en: {
        language: 'English',
        welcome: `Welcome to ${eventName} utility!`,
        ...
        actions: {
            ...
            list: {
                ...
                jsonScores: {
                    title: `Download itl${eventYear}.json file`,
                    description:
                        'Download your event data json file (to put on your profile folder)',
                },
                excelCompare: {
                    title: `Download players compare as excel file for ${eventName}`,
                    description:
    ...
    it: {
        language: 'Italiano',
        welcome: `Benvenuto in ${eventName} utility!`,
        ...
        actions: {
            ...
            list: {
                ...
                jsonScores: {
                    title: `Download file ITL${eventYear}.json`,
                    description:
                        'Scarica i tuoi score come json file (da mettere nel tuo profilo)',
                },
                excelCompare: {
                    title: `Scarica l'excel di comparazione score/punteggi con un player per l'evento ${eventName}`,
                    description:
...
export async function setLanguage() {
    const langCodes = Object.keys(labels);
    const languageList: { name: string; value: string }[] = [];

    langCodes.forEach((code) => {
        languageList.push({
            name: labels[code as Language].language as string,
            value: code,
        });
    });

    const userLang = await select({
        message: 'Select language:',
        choices: languageList,
    });

    language = userLang as Language;
}
...
export function getTranslatedLabel(
    ...
    let result = labelPath.split('.').reduce<unknown>((acc, key) => {
        if (acc && typeof acc === 'object') {
            return (acc as LanguageLabel)[key];
        }
        return undefined;
    }, dictionary);
    ...
    for (const [key, value] of Object.entries(replacements)) {
        result = (result as string).replaceAll(`#${key}#`, value);
    }
